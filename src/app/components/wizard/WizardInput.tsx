...
export const WizardInput: React.FC<WizardInputProps> = ({
  ...
  return (
    <div
      data-print-hide
      className="flex-none p-4 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-t border-gray-100 dark:border-zinc-800 z-20 pb-safe"
    >
      <div className="max-w-4xl mx-auto w-full relative">
        ...
        {awaitingPlanConfirmation && onConfirmGenerate && onGoDeeper && (
          <div
            role="region"
            aria-label={
              t("wizard.approvalGateLabel") ||
              "Ready to generate plan. Choose an action."
            }
            aria-live="polite"
          ...
          >
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
              {t("wizard.planReminderShort") ||
                t("wizard.personalizedPlanReminder") ||
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
              <button
                ...
                data-testid="wizard-approval-confirm"
                disabled={isTyping || isAgentRunning}
                aria-label={
                  t("wizard.generatePlanAria") ||
              ...
              <button
                type="button"
                onClick={onGoDeeper}
                data-testid="wizard-approval-deeper"
                disabled={isTyping || isAgentRunning}
                aria-label={
                  t("wizard.goDeeperAria") ||
                  t("wizard.notReadyGoDeeper") ||
                  "Not ready yet, keep exploring with more questions"
                }
                title={
                  t("wizard.notReadyGoDeeper") ||
              ...
              >
                <span className="sm:hidden truncate">
                  {t("wizard.goDeeperShort") || "Go deeper"}
                </span>
        ...
        <form onSubmit={onSubmit} className="w-full relative">
          ...
          <button
            ...
            aria-label={
              expanded ? t("wizard.inputCollapse") : t("wizard.inputExpand")
            }
            title={
              expanded ? t("wizard.inputCollapse") : t("wizard.inputExpand")
            }
          ...
          <button
            ...
            disabled={
              !inputValue.trim() || isTyping || isAgentRunning || isOffline
            }
            className={`absolute right-2 w-10 h-10 rounded-xl flex items-center justify-center transition-all ${expanded ? "top-6" : "top-1/2 -translate-y-1/2"} ${
              (!inputValue.trim() && !isAgentRunning) || isOffline
                ? "bg-gray-200 text-gray-400 dark:bg-zinc-800 dark:text-zinc-600 cursor-not-allowed"
                : "cursor-pointer bg-black dark:bg-white text-white dark:text-black hover:scale-105 active:scale-95"
            }`}
          >
            <Send size={20} />
          </button>
