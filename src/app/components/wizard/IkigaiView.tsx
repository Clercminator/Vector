    ...
    >
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          title={title ? `${copySectionLabel}: ${title}` : copySectionLabel}
          aria-label={
            title ? `${copySectionLabel}: ${title}` : copySectionLabel
          }
        >
          <Copy size={18} />
        </button>
...
export const IkigaiView: React.FC<IkigaiViewProps> = ({
  result,
  updateResult,
}) => {
  const { t } = useLanguage();
  const isMobile = useIsMobile();
  const copySectionLabel = t("common.copySection");
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <IkigaiSection
            title={t("ikigai.love")}
            icon={
              <Heart className="text-rose-500 fill-rose-500/20" size={24} />
            }
            value={result.love}
          ...
          <IkigaiSection
            ...
            icon={
              <Globe className="text-blue-500 fill-blue-500/20" size={24} />
            }
            value={result.worldNeeds}
            onChange={(val) => updateResult(["worldNeeds"], val)}
            onCopy={() =>
              handleCopySection(result.worldNeeds, t("ikigai.worldNeeds"))
            }
          ...
          <IkigaiSection
            ...
            onCopy={() =>
              handleCopySection(result.paidFor, t("ikigai.paidFor"))
            }
