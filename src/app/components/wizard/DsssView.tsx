...
const STEPS = [
  {
    key: "deconstruct",
    icon: Layers,
    labelKey: "dsss.deconstruct",
    descKey: "fw.dsss.definition",
    color: "orange",
  },
  {
    key: "selection",
    icon: Filter,
    labelKey: "dsss.selection",
    descKey: null,
    color: "amber",
  },
  {
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <React.Fragment key={step.key}>
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full ${colors.bg} border ${colors.border} text-sm font-bold`}
                >
                  <span
                    className={`w-6 h-6 rounded-full ${colors.bar} text-white flex items-center justify-center text-xs`}
                  >
                    {i + 1}
                  </span>
                  <Icon
                ...
                {i < STEPS.length - 1 && (
                  <div
                    className="hidden sm:block w-4 h-0.5 bg-orange-300 dark:bg-orange-700 rounded"
                    aria-hidden
                  />
                )}
              </React.Fragment>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                1. {t("dsss.deconstruct")}
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {t("dsss.desc.deconstruct")}
              </p>
            </div>
          </div>
          <EditableList
