...
export const generatePdf = async (
  ...
  const setDrawPrimary = () =>
    doc.setDrawColor(primaryRgb[0], primaryRgb[1], primaryRgb[2]);
  const setDrawBorder = () =>
    doc.setDrawColor(
      VECTOR_THEME.border[0],
      VECTOR_THEME.border[1],
      VECTOR_THEME.border[2],
    );
  ...
  const frameworkLabel = String(blueprint.framework || "")
    .replace(/-/g, " ")
  ...
  if (frameworkLabel)
    doc.text(
      frameworkLabel,
      centerX - doc.getTextWidth(frameworkLabel) / 2,
      coverY,
    );
  coverY += 10;
  ...
  doc.text(
    ...
    centerX -
      doc.getTextWidth(new Date(blueprint.createdAt).toLocaleDateString()) / 2,
    coverY,
  );
  coverY += 24;
  doc.setFontSize(10);
  ...
  const personalizedText = userName
    ? getLabel(
        lang,
        "personalizedPlanFor",
        "Personalized plan for {0}",
        userName,
      )
    : getLabel(lang, "personalizedPlan", "Your personalized plan");
  doc.text(
    personalizedText,
    centerX - doc.getTextWidth(personalizedText) / 2,
    coverY,
  );
  ...
  if (result?.executiveSummary) {
    renderSection(
      getLabel(lang, "executiveSummary", "Executive Summary"),
      result.executiveSummary,
    );
  }

  if (result?.currentReality) {
    renderSection("Strategic Diagnosis", result.currentReality);
  }

  if (
    Array.isArray(result.strategicPillars) &&
    result.strategicPillars.length
  ) {
    renderSection(
      "Strategic Pillars",
      result.strategicPillars.map((item: string) => `• ${item}`),
    );
  }

  if (Array.isArray(result.keyConstraints) && result.keyConstraints.length) {
    renderSection(
      "Constraint Map",
      result.keyConstraints.map((item: string) => `• ${item}`),
    );
  }

  if (Array.isArray(result.leverageMoves) && result.leverageMoves.length) {
    renderSection(
      "Leverage Moves",
      result.leverageMoves.map((item: string) => `• ${item}`),
    );
  }
  ...
  if (result?.difficulty || result?.difficultyReason || result?.commitment) {
    ensureSpace(10);
    recordToc(
      getLabel(lang, "difficultyCommitment", "Difficulty & Commitment"),
    );
    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    setPrimary();
    ...
    if (result.difficulty)
      parts.push(
        `Level: ${String(result.difficulty).replace(/\b\w/g, (c) => c.toUpperCase())}`,
      );
  ...
  const goalMatch = firstAnswer.match(
    /(?:Goal|Objective|Objetivo)\s*:?\s*([^.]+)/i,
  );
  if (goalMatch?.[1]?.trim()) {
    situationParts.push(`Goal: ${goalMatch[1].trim()}`);
  } else if (/Stakes?|Time horizon|Horizonte|Obstacles?/i.test(firstAnswer)) {
    const before = firstAnswer
      .split(/\s*Stakes?|Time horizon|Horizonte\s*:?\s*/i)[0]
      ?.trim();
    if (before?.length > 5) situationParts.push(`Goal: ${before}`);
  }
  const stakesMatch = firstAnswer.match(/(?:Stakes?|Apuesta)\s*:?\s*([^.]+)/i);
  const horizonMatch = firstAnswer.match(
    /(?:Time horizon|Horizon|Horizonte temporal)\s*:?\s*([^.]+)/i,
  );
  const obstaclesMatch = firstAnswer.match(
    /(?:Obstacles?|Obstáculos?|Constraints?)\s*:?\s*([^.]+)/i,
  );
  if (goalMatch?.[1]?.trim())
    situationParts.push(`Goal: ${goalMatch[1].trim()}`);
  if (stakesMatch?.[1]?.trim())
    situationParts.push(`Stakes: ${stakesMatch[1].trim()}`);
  if (horizonMatch?.[1]?.trim())
    situationParts.push(`Timeline: ${horizonMatch[1].trim()}`);
  if (obstaclesMatch?.[1]?.trim())
    situationParts.push(`Constraints: ${obstaclesMatch[1].trim()}`);
  if (situationParts.length === 0 && firstAnswer.trim()) {
    situationParts.push(
      firstAnswer.slice(0, 300) + (firstAnswer.length > 300 ? "…" : ""),
    );
  }
  if (situationParts.length > 0) {
    renderSection(
      getLabel(lang, "yourSituation", "Your Situation"),
      situationParts,
    );
  }
  ...
  const yourWhy =
    result?.yourWhy ||
    (result?.type === "rpm" && result?.purpose ? result.purpose : null);
  if (yourWhy) {
  ...
  if (Array.isArray(firstWeek) && firstWeek.length > 0) {
    renderSection(
      getLabel(lang, "firstWeekActions", "First Week Actions"),
      firstWeek.map((a: string, i: number) => `${i + 1}. ${a}`),
    );
  }
  ...
  if (Array.isArray(milestones) && milestones.length > 0) {
    renderSection(
      getLabel(lang, "milestones", "Milestones"),
  ...
  const successMatch =
    !successCriteria && firstAnswer
      ? firstAnswer.match(/(?:Success looks like|Éxito)\s*:?\s*([^.]+)/i)
      : null;
  const successText =
    successCriteria ||
    (successMatch?.[1]?.trim()
      ? `You'll know you've succeeded when: ${successMatch[1].trim()}`
      : null);
  if (successText) {
    renderSection(
      getLabel(lang, "successCriteria", "Success Criteria"),
      successText,
    );
  }
  ...
  if (avoidItems.length > 0) {
    renderSection(
      getLabel(lang, "whatToAvoid", "What to Avoid"),
      avoidItems.map((a) => `• ${a}`),
    );
  }

  if (Array.isArray(result.failureModes) && result.failureModes.length > 0) {
    renderSection(
      "Failure Modes",
      result.failureModes.map((item: string) => `• ${item}`),
    );
  }

  if (
    Array.isArray(result.resourceChecklist) &&
    result.resourceChecklist.length > 0
  ) {
    renderSection(
      "Setup Checklist",
      result.resourceChecklist.map((item: string) => `• ${item}`),
    );
  }

  if (Array.isArray(result.decisionRules) && result.decisionRules.length > 0) {
    renderSection(
      "Decision Rules",
      result.decisionRules.map((item: string) => `• ${item}`),
    );
  }

  if (
    Array.isArray(result.leadIndicators) &&
    result.leadIndicators.length > 0
  ) {
    renderSection(
      "Lead Indicators",
      result.leadIndicators.map((item: string) => `• ${item}`),
    );
  }

  if (Array.isArray(result.lagIndicators) && result.lagIndicators.length > 0) {
    renderSection(
      "Lag Indicators",
      result.lagIndicators.map((item: string) => `• ${item}`),
    );
  }

  if (
    Array.isArray(result.proofChecklist) &&
    result.proofChecklist.length > 0
  ) {
    renderSection(
      "Proof Checklist",
      result.proofChecklist.map((item: string) => `• ${item}`),
    );
  }

  if (result.recoveryProtocol) {
    renderSection("Recovery Protocol", result.recoveryProtocol);
  }

  if (Array.isArray(result.scheduleHints) && result.scheduleHints.length > 0) {
    renderSection(
      "Schedule Snapshot",
      result.scheduleHints.map((hint: any) => {
        const parts = [hint.label, hint.cadence];
        if (Array.isArray(hint.days) && hint.days.length)
          parts.push(`days: ${hint.days.join(", ")}`);
        if (hint.time) parts.push(`time: ${hint.time}`);
        if (hint.durationMinutes) parts.push(`${hint.durationMinutes} min`);
        return parts.join(" | ");
      }),
    );
  }
  ...
  if (
    Array.isArray(result.ownershipCadence) &&
    result.ownershipCadence.length > 0
  ) {
    renderSection(
      "Ownership Cadence",
      result.ownershipCadence.map((item: string) => `• ${item}`),
    );
  }

  if (result.trackingPrompt) {
    renderSection("Tracking Prompt", result.trackingPrompt);
  }

  if (Array.isArray(result.supportSystem) && result.supportSystem.length > 0) {
    renderSection(
      "Support System",
      result.supportSystem.map((item: string) => `• ${item}`),
    );
  }

  if (
    Array.isArray(result.accountabilityHooks) &&
    result.accountabilityHooks.length > 0
  ) {
    renderSection(
      "Accountability Commitments",
      result.accountabilityHooks.map((item: string) => `• ${item}`),
    );
  }

  if (
    Array.isArray(result.revisionTriggers) &&
    result.revisionTriggers.length > 0
  ) {
    renderSection(
      "Revision Triggers",
      result.revisionTriggers.map((item: string) => `• ${item}`),
    );
  }

  if (result.weeklyReviewPrompt) {
    renderSection("Weekly Review Prompt", result.weeklyReviewPrompt);
  }
  ...
  if (blueprint.result) {
    ...
    if (result.type === "first-principles") {
      ...
      renderQuadrant(
        getLabel(lang, "urgentImportant", "Urgent & Important (Do)"),
        result.q1 || [],
      );
      renderQuadrant(
        getLabel(lang, "importantNotUrgent", "Important Not Urgent (Schedule)"),
        result.q2 || [],
      );
      renderQuadrant(
        getLabel(lang, "urgentNotImportant", "Urgent Not Important (Delegate)"),
        result.q3 || [],
      );
      renderQuadrant(
        getLabel(lang, "neither", "Neither (Eliminate)"),
        result.q4 || [],
      );
      ...
      result.type === "mandalas" ||
      (blueprint.framework === "mandalas" &&
        (result.centralGoal ||
          result.central_goal ||
          (result.categories && result.categories.length > 0)))
    ) {
      const mandala = result as {
        centralGoal?: string;
        central_goal?: string;
        categories?: Array<{ name: string; steps?: string[]; why?: string }>;
      };
      const centralGoalText = (
        mandala.centralGoal ||
        mandala.central_goal ||
        blueprint.title ||
        ""
      ).trim();
      doc.setFont("helvetica", "bold");
      ...
      categories.forEach(
        (cat: { name?: string; steps?: string[]; why?: string }, i: number) => {
          ensureSpace(10);
          doc.setFont("helvetica", "bold");
          setPrimary();
      ...
      const toStr = (v: unknown): string =>
        Array.isArray(v)
          ? (v as string[]).map(String).filter(Boolean).join(" ")
          : typeof v === "string"
            ? v
            : String(v ?? "");
      doc.setFont("helvetica", "bold");
      ...
      y += 8;
      doc.setFont("helvetica", "normal");
      const challengeLines = doc.splitTextToSize(
        toStr(result.challenge),
        contentWidth,
      );
      ...
      y += 8;
      doc.setFont("helvetica", "normal");
      const purifLines = doc.splitTextToSize(
        toStr(result.purification),
        contentWidth,
      );
  ...
  if (Array.isArray(messages) && messages.length > 0) {
    ...
    for (const m of messages) {
      if (m.role === "ai" && m.content) {
        lastAi = stripMarkdown(
          m.content
            .replace(/\|\|\[.*?\]\s*$/s, "")
            .replace(/\|\|DRAFT_START[\s\S]*\|\|DRAFT_END\|\|/g, "")
            .trim(),
        );
      }
    ...
    const realAnswers = (blueprint.answers || []).filter(
      (a) => !isFakeAnswer(a),
    );
  ...
  if (tocEntries.length > 0) {
    ...
    for (const entry of tocEntries) {
      ...
      if (typeof textWithLink === "function") {
        try {
          textWithLink.call(doc, entry.name, margin, tocY, {
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.text(
      `Page ${p} of ${pageCount}`,
      pageWidth - margin - doc.getTextWidth(`Page ${p} of ${pageCount}`),
      pageHeight - 12,
    );
  }
