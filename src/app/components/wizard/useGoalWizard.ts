...
export const useGoalWizard = ({
  ...
  const ensureMessageIds = (raw: any[]): Message[] => {
    ...
    return raw
      ...
      .filter(
        (m) => m.role === "ai" || m.role === "user" || m.role === "system",
      );
  };
  ...
  const [awaitingPlanConfirmation, setAwaitingPlanConfirmation] =
    useState(false);
  ...
  const [agentPhase, setAgentPhase] = useState<
    "thinking" | "drafting" | "reviewing" | "finalizing"
  >("thinking");
  ...
  useEffect(() => {
    ...
    if (initialBlueprint?.id && supabase) {
      channel = supabase
        .channel(`blueprint-${initialBlueprint.id}`)
        .on(
          "postgres_changes",
          {
            event: "UPDATE",
          ...
          () => {
            if (!isTyping && !isAgentRunning) {
              toast.info(
                t("wizard.externalUpdate") ||
                  "This blueprint was updated in another tab.",
                {
                  action: {
                    label: "Refresh",
    ...
    if (savedSession) {
      try {
        const {
          ...
          framework: savedFw,
          timestamp,
          awaitingPlanConfirmation: savedAwaiting,
  ...
  useEffect(() => {
        ...
        fetchBlueprintMessages(supabase, initialBlueprint.id)
          .then((history: any[]) => {
            if (history && history.length > 0)
              setMessages([...baseMessages, ...ensureMessageIds(history)]);
          })
          .catch((err: any) =>
    ...
    if (savedSession) {
      try {
        ...
        if (
          Date.now() - (timestamp || 0) < 24 * 60 * 60 * 1000 &&
          savedFw === framework
        )
          return;
      } catch (_) {}
    ...
    const initialContext =
      window.history.state?.usr?.context || (location.state as any)?.context;
  ...
  useEffect(() => {
    ...
    if (ctx.successLookLike)
      parts.push(`Success looks like: ${ctx.successLookLike}`);
    ...
    if (ctx.successLookLike)
      formCtxParts.push(`Success looks like: ${ctx.successLookLike}`);
    if (ctx.explanation)
      formCtxParts.push(`Why this framework fits: ${ctx.explanation}`);
    ...
    supabase.auth
      ...
      .then(({ data: { user } }: { data: { user: any } }) => {
        ...
        supabase
          .from("profiles")
          .select(
            "credits,extra_credits,credits_expires_at,tier,display_name,bio,metadata",
          )
          .eq("user_id", user.id)
          .single()
          .then(({ data }: { data: any }) => {
            if (data) {
              ...
              if (meta.zodiac_importance)
                parts.push(
                  `Zodiac importance for personality: ${meta.zodiac_importance}`,
                );
              ...
              if (meta.preferred_plan_style)
                parts.push(
                  `Preferred plan style: ${meta.preferred_plan_style}`,
                );
              if (meta.stay_on_track)
                parts.push(
                  `What helps them stay on track: ${meta.stay_on_track}`,
                );
              if (meta.question_flow)
                parts.push(
                  `Prefers questions: ${meta.question_flow === "list" ? "all at once" : "one at a time with suggestions"}`,
                );
              if (meta.preferred_tone)
                parts.push(`Preferred tone: ${meta.preferred_tone}`);
              if (meta.treatment_level)
                parts.push(
                  `Treat as: ${meta.treatment_level} (expert=assume knowledge, beginner=explain, mixed=adapt)`,
                );
              if (meta.other_observations)
                parts.push(`Other observations: ${meta.other_observations}`);
  ...
  useEffect(() => {
    const ctx =
      window.history.state?.usr?.context ||
      (
        location.state as {
          context?: {
            objective?: string;
    ...
    if (ctx.successLookLike)
      parts.push(`Success looks like: ${ctx.successLookLike}`);
    if (ctx.explanation)
      parts.push(`Why this framework fits: ${ctx.explanation}`);
  ...
  useEffect(() => {
    ...
    if (!isPreviewMode && !canUseFramework(tier, framework)) {
      toast.error(
        t("wizard.lockedError") ||
          "This framework is not available in your plan.",
        { duration: 15000 },
      );
      onBack();
    }
  ...
  const toggleListening = () => {
    ...
    recognition.onresult = (event: any) => {
      setInputValue((prev) =>
        (prev + " " + event.results[0][0].transcript).trim(),
      );
    };
  ...
  const extractContent = (
    text: string,
  ): { cleanText: string; suggestions: string[]; draft: any | null } => {
    let cleanText = text;
    ...
    if (startIndex !== -1) {
      ...
      if (endIndex !== -1) {
        ...
        cleanText =
          ...
          afterStart.substring(endIndex + endTag.length);
      } else {
        jsonStr = afterStart;
  ...
  const runE2EResume = async (payload: {
    confirmed: boolean;
    userMessage?: string;
  }) => {
    const updatedMessages = payload.confirmed
      ? messages
      : payload.userMessage
        ? [
            ...messages,
    ...
    setAwaitingPlanConfirmation(false);
    if (!payload.confirmed && payload.userMessage) {
      setMessages((prev) => [
        ...prev,
        {
          id: uuidv4(),
    ...
    try {
      ...
      await wait(200);
      const blueprint = buildE2EWizardResult(framework, userAnswers);
      setDraftResult(blueprint);
      setAgentPhase("reviewing");
      await wait(150);
      setResult(blueprint);
  ...
  const runAgentInternal = async (
    ...
    if (isE2EMode()) {
      return runE2ETurn(userText, options);
    }
    ...
    if (credits !== null && credits <= 0) {
      toast.error(t("wizard.outOfCredits") || "Low credits.", {
    ...
    const looksLikeConfirmation =
      /^(yes|sí|si|ok|okay|looks good|perfect|proceed|generate|go ahead|listo|vale|adelante|dale|genera|genéralo|génère)/i.test(
        userText.trim(),
      );
    ...
    try {
      ...
      const inputs = {
        messages: seeded
          ? [...seeded, new HumanMessage(userText)]
      ...
      LOG("runAgent START", {
        ...
        framework,
        currentLang,
        goalLength: userText.length,
        seededCount: seeded?.length ?? 0,
      });
      ...
      for await (const event of generator) {
        ...
        if (!isMounted.current || !isRunningRef.current) {
          LOG("break: unmounted or agent stopped", {
        ...
        const updateKeys =
          updateData && typeof updateData === "object"
            ...
            : [];
        LOG(`event #${eventIndex}`, {
          isTuple,
          streamMode,
          updateKeys,
          hasMessages: !!updateData?.messages,
          messagesLength: updateData?.messages?.length,
        });
        ...
        const hasInterrupt = (data: any): boolean => {
          ...
          try {
            if (typeof isInterrupted === "function" && isInterrupted(data))
              return true;
            const key =
              typeof INTERRUPT !== "undefined" ? INTERRUPT : "__interrupt__";
        ...
        if (
          streamMode === "updates" &&
          updateKeys.includes("user_review") &&
          isMounted.current
        ) {
        ...
        if (
          ...
          if (!blueprintFromStream) {
            for (const key of Object.keys(updateData)) {
              ...
              if (
                nodeOut &&
                typeof nodeOut === "object" &&
                (nodeOut as any).blueprint
              ) {
        ...
        if (
          !blueprintFromStream &&
          streamMode === "values" &&
          seenDraftUpdateThisRunRef.current &&
        ...
        if (
          blueprintFromStream &&
          typeof blueprintFromStream === "object" &&
          blueprintFromStream.type &&
          !(blueprintFromStream as any).isTeaser
        ) {
          ...
          if (isMounted.current) {
            ...
            if (
              ...
              for (const key of Object.keys(updateData)) {
                ...
                if (
                  nodeOut &&
                  typeof nodeOut === "object" &&
                  Array.isArray((nodeOut as any).messages)
                ) {
            ...
            if (supabase && !hasDeductedCreditThisRunRef.current) {
              ...
              supabase.auth
                ...
                .then(({ data: { user } }) => {
                  if (user?.id) {
                    supabase
                      .rpc("decrement_credits", { amount_to_deduct: 1 })
                      .then(({ data, error }) => {
            ...
            trackEvent("wizard_completed", {
              framework: framework || bp.framework,
            });
            setWizardContext({ completed: true });
          }
        }
        ...
        if (streamMode === "values" && updateData?.messages) {
          LOG(`event #${eventIndex} values.messages`, {
            count: updateData.messages.length,
            lastRole:
              updateData.messages[updateData.messages.length - 1]?.role ??
              updateData.messages[updateData.messages.length - 1]?.type,
          });
        }
        if (streamMode !== "values" && updateKeys.length > 0) {
          for (const k of updateKeys) {
            ...
            if (
              ...
              LOG(`event #${eventIndex} nested messages under key "${k}"`, {
                count: (v as any).messages.length,
              });
              break;
            }
          }
        ...
        const frameworkSetter =
          updateData?.framework_setter ||
          (Array.isArray(event) &&
          event?.[0] === "framework_setter" &&
          event?.[1]
            ? event[1]
            : null);
        if (frameworkSetter) {
        ...
        if (
          streamMode === "values" &&
          updateData?.messages &&
          Array.isArray(updateData.messages)
        ) {
        ...
        if (messageList && messageList.length > 0) {
          ...
          const getMsgType = (m: any) => {
            ...
            return (m as any).constructor?.name === "AIMessage"
              ? "ai"
              : undefined;
          };
          ...
          if (lastMsgType !== "ai") {
            const aiIdx = messageList
              .map((m: any) => getMsgType(m))
              .lastIndexOf("ai");
          ...
          const isTool =
            ...
            (lastMsg?.name &&
              ["set_framework", "request_confirmation"].includes(lastMsg.name));
          // If the model called our confirmation tool, surface the approval-gate UI
          const toolCalls = (lastMsg as any)?.tool_calls;
          if (
            Array.isArray(toolCalls) &&
            toolCalls.some((tc: any) => tc?.name === "request_confirmation")
          ) {
            if (isMounted.current) {
              setAwaitingPlanConfirmation(true);
              LOG(
                `event #${eventIndex} request_confirmation detected → awaitingPlanConfirmation=true`,
              );
            }
          }
          const isAIMessage =
          ...
          LOG(`event #${eventIndex} messageList`, {
            source: messageListSource,
            count: messageList.length,
            msgType,
            isTool,
            isAIMessage,
            hasContent: lastMsg?.content != null,
            contentPreview,
          });
          ...
          if (isAIMessage && lastMsg?.content != null) {
            ...
            if (rawContent == null) {
              LOG(
                `event #${eventIndex} SKIP: normalizeMessageContent returned null`,
              );
              continue;
            }
            const { cleanText, suggestions, draft } =
              extractContent(rawContent);
            let textToShow = (
              cleanText.trim() || rawContent.trim().slice(0, 2000)
            ).trim();
            ...
            textToShow = textToShow
              .replace(
                /\n\s*(\*\*)?Ready to generate the full blueprint[^*]*(\*\*)?\s*$/i,
                "",
              )
              .trim();
            if (
              /^Your .+ blueprint is ready below\. Review your personalized plan and refine as needed\.$/.test(
                textToShow,
              ) ||
              textToShow ===
              ...
              const label =
                ...
                "";
              const translated = (t("wizard.blueprintReadyBelow") || "")
                .replace("{0}", label)
                .trim();
            ...
            const isLoadingPlaceholder =
              !textToShow ||
              loadingPlaceholders.includes(textToShow) ||
              textToShow === (t("common.loading") ?? "Loading...");
            LOG(`event #${eventIndex} WILL setMessages`, {
              rawContentLength: rawContent.length,
              cleanTextLength: cleanText.length,
              textToShowPreview: textToShow.slice(0, 120),
              suggestionsCount: suggestions.length,
            });

            if (lastAppendedContentRef.current === textToShow) {
              LOG(
                `event #${eventIndex} setMessages: no-op (duplicate, ref check)`,
              );
            } else if (!textToShow || isLoadingPlaceholder) {
              LOG(
                `event #${eventIndex} setMessages: no-op (empty or loading placeholder)`,
              );
            } else {
              setMessages((prev) => {
                const alreadyExists = prev.some(
                  (m) => m.role === "ai" && m.content === textToShow,
                );
                if (alreadyExists) {
                  LOG(
                    `event #${eventIndex} setMessages: no-op (already in state, from checkpoint)`,
                  );
                  return prev;
                }
                ...
                if (
                  lastPrev &&
                  lastPrev.role === "ai" &&
                  lastPrev.content === textToShow
                ) {
                  LOG(
                    `event #${eventIndex} setMessages: no-op (duplicate, state check)`,
                  );
                  return prev;
                }
                const isGreetingOnly =
                  /^[¡]?Hola!?\s*\.?$/i.test(textToShow.trim()) ||
                  /^Hello!?\s*\.?$/i.test(textToShow.trim());
                if (
                  isGreetingOnly &&
                  lastPrev?.role === "ai" &&
                  /^[¡]?Hola!?|^Hello!?/i.test(String(lastPrev.content).trim())
                ) {
                  LOG(
                    `event #${eventIndex} setMessages: no-op (duplicate greeting)`,
                  );
                  return prev;
                }
                lastAppendedContentRef.current = textToShow;
                LOG(`event #${eventIndex} setMessages: APPENDING ai message`, {
            ...
            if (messageList.length > 0 && !isAIMessage) {
              LOG(`event #${eventIndex} SKIP: not treating as AI`, {
                msgType,
                isTool,
                hasContent: lastMsg?.content != null,
              });
          ...
          if (eventIndex <= 5 || updateKeys.length > 0) {
            LOG(`event #${eventIndex} no messageList`, {
              streamMode,
              updateKeys,
              hasUpdateDataMessages: !!updateData?.messages,
            });
          }
        }
      }

      LOG("stream loop DONE", {
      ...
      if (
        ...
        console.warn(
          "[Wizard:Agent] Agent finished but no AI messages received.",
        );
        setMessages((prev) => [
          ...prev,
          {
  ...
  const runAgentResume = async (payload: {
    confirmed: boolean;
    userMessage?: string;
  }) => {
    if (isE2EMode()) {
      return runE2EResume(payload);
    }
    ...
    try {
      ...
      for await (const event of generator) {
        ...
        if (frameworkSetter) {
          ...
          if (newFw && newFw !== framework && onSwitchFramework) {
            setDraftResult(null);
            onSwitchFramework(
              newFw as FrameworkId,
              !canUseFramework(tier, newFw as FrameworkId),
            );
          }
        }
        ...
        if (messageList && messageList.length > 0) {
          ...
          if (isAIMessage && lastMsg?.content != null) {
            ...
            if (lastAppendedContentRef.current !== textToShow && textToShow) {
              ...
              setMessages((prev) => {
                if (
                  prev.some((m) => m.role === "ai" && m.content === textToShow)
                )
                  return prev;
                return [
                  ...prev,
      ...
      let errorMsg =
        t("errors.agentGeneric") ||
        "We couldn't complete your plan. Please try again.";
      if (
  ...
  const editUserMessageOnce = async (messageId: string, newContent: string) => {
    ...
    if (isRunningRef.current || isTyping || isAgentRunning) {
      toast.info(
        t("wizard.waitUntilDone") || "Wait for the agent to finish, then edit.",
      );
      return;
    }
    ...
    if (!latestUserMessageId || latestUserMessageId !== messageId) {
      toast.info(
        t("wizard.editLatestOnly") || "You can only edit your latest message.",
      );
      return;
    }
  ...
  const promoteDraftToResult = () => {
    ...
    trackEvent("wizard_completed", {
      framework: framework || (draftResult as any).framework,
    });
    setWizardContext({ completed: true });
  };
  ...
  const handleSave = async () => {
    const raw = finalAnswers.length
      ...
      : messages.filter((m) => m.role === "user").map((m) => m.content);
    const answers = filterRealAnswers(raw);
    const effectiveResult = normalizeCanonicalPlanResult(
      (result ??
        draftResult ?? {
          type: (framework || "general") as FrameworkId,
        }) as Record<string, unknown>,
      framework || String(result?.type || draftResult?.type || "general"),
      {
        title: initialBlueprint?.title ?? blueprintTitleFromAnswers(answers),
        goal: answers[0],
      },
    ...
    const validation = validateCanonicalPlanResult(
      effectiveResult,
      framework || String(effectiveResult.type || "general"),
    );
    if (!validation.isValid) {
      throw new Error(validation.errors[0] || "Plan validation failed");
    }
    const bp: Blueprint = {
      id: initialBlueprint?.id ?? crypto.randomUUID(),
      framework:
        framework ||
        (result?.type as FrameworkId) ||
        (draftResult?.type as FrameworkId) ||
        "general",
      title: initialBlueprint?.title ?? blueprintTitleFromAnswers(answers),
      answers,
    ...
    try {
      ...
      if (supabase) {
        ...
        syncBlueprintMessages(supabase, bp.id, persistable).catch((err: any) =>
  ...
  const toggleHardMode = () => {
    ...
    toast(
      isHardMode
        ? "Mode: Supportive Coach 🤝"
        : "Mode: Devil's Advocate (Hard Mode) 🔥",
      {
        description: !isHardMode
          ? "The agent will now be ruthless and critical."
          : "The agent is back to being nice.",
      },
    );
  };
