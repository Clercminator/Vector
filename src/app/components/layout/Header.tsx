        ...
        <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-gray-100 dark:border-zinc-800 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-6">
                    ...
                    {userEmail && (
                        <>
                            <button onClick={() => navigate('/today')} className={`text-sm font-medium transition-colors cursor-pointer ${location.pathname === '/today' ? 'text-black dark:text-white font-semibold' : 'text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white'}`} aria-current={location.pathname === '/today' ? 'page' : undefined}>{t('tracker.today') || 'Today'}</button>
