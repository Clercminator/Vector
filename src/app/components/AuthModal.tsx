...
export function AuthModal({
  ...
  const [emailSentState, setEmailSentState] = useState<{
    email: string;
    type: "signup" | "magic_link" | "forgot_password";
  } | null>(null);
    ...
    try {
      ...
      if (mode === "signin") {
        ...
        if (!error) {
          ...
          toast.success(
            t("auth.success") || "Magic link sent! Check your inbox.",
            { duration: 15000 },
          );
        }
      ...
      toast.error(
        e.message || t("auth.authenticationFailed") || "Authentication failed",
        { duration: 15000 },
      );
      captchaRef.current?.resetCaptcha();
      setCaptchaToken(null);
          ...
          {emailSentState ? (
            <div className="rounded-2xl border border-green-200 bg-green-50 dark:bg-green-900/20 dark:border-green-800/50 p-6 space-y-3">
              ...
              <p className="text-sm text-green-600 dark:text-green-300">
                {emailSentState.type === "signup" &&
                  (t("auth.checkEmailConfirmDesc") ||
                    "We've sent a confirmation link to your email. Click it to activate your account.")}
                {emailSentState.type === "magic_link" &&
                  (t("auth.magicLinkSentDesc") ||
                    "We've sent a sign-in link to your email. Click it to sign in.")}
                {emailSentState.type === "forgot_password" &&
