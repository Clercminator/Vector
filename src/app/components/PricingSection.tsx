...
export const PricingSection: React.FC<{
  ...
  const tiers = tierIds.map((id) => {
    ...
    const features: string[] =
      id === "architect"
        ? [
            t("pricing.feature.architect.plans").replace(
              "{0}",
              String(config.credits),
            ),
        ...
        : id === "builder"
          ? [
              t("pricing.feature.builder.plans").replace(
                "{0}",
                String(config.credits),
              ),
          ...
          : id === "max"
            ? [
                t("pricing.feature.max.plans").replace(
                  "{0}",
                  String(config.credits),
                ),
                ...
                t("pricing.feature.max.tracker"),
                t("pricing.feature.max.support"),
          ...
          <div className="mt-6 flex items-center justify-center gap-2 flex-wrap">
            {isPaymentRegionLoading ? (
              <span className="text-sm text-gray-400">
                {t("pricing.regionDetecting") || "Detecting region..."}
              </span>
            ) : (
              <>
                <span className="text-sm text-gray-500 dark:text-gray-400">
                  {paymentRegion === "latam"
                    ? t("pricing.regionLatam") ||
                      "Showing LATAM payment (MercadoPago)"
                    : t("pricing.regionGlobal") ||
                      "Showing US/EU payment (Cards, PayPal)"}
                </span>
                <button
                  type="button"
                  onClick={() =>
                    setPaymentRegion(
                      paymentRegion === "latam" ? "global" : "latam",
                    )
                  }
        ...
        {tiers.map((tier) => (
          <motion.div
            ...
            className={`relative p-8 rounded-3xl border ${
              tier.popular
                ? "border-black dark:border-white shadow-2xl scale-105 z-10"
                : tier.isCurrent
          ...
          >
            ...
            <div className="space-y-3">
              ...
              >
                {(tier.id === "builder" || tier.id === "max") &&
                  !tier.isCurrent && <CreditCard size={18} />}
                {tier.cta}
              </button>
              {(tier.id === "builder" || tier.id === "max") &&
                !tier.isCurrent &&
