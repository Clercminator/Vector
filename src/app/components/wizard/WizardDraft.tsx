  ...
  return (
    <div className="flex items-center gap-3 bg-white dark:bg-zinc-800/50 px-3 py-1.5 rounded-lg border border-gray-100 dark:border-zinc-800/50">
      <div className="flex flex-col items-end">
        ...
        <span className={`text-xl font-mono leading-none ${color}`}>
          {score}%
        </span>
      </div>
...
export const WizardDraft: React.FC<WizardDraftProps> = ({
  ...
  const renderGoalMri = (mri: GoalMri) => {
    ...
    const renderMriList = (
      ...
      <div className={`rounded-xl border p-3 ${tone}`}>
        <h5 className="text-xs font-bold uppercase tracking-[0.18em] text-gray-500 mb-2">
          {t(labelKey)}
        </h5>
        {items.length > 0 ? (
          <ul className="space-y-1.5">
            {items.map((item) => (
              <li
                key={item}
                className="text-sm text-gray-700 dark:text-gray-300 leading-snug break-words"
              >
                {item}
              </li>
            ))}
    ...
    return (
      <div
        className={`rounded-2xl border p-4 md:p-5 ${awaitingPlanConfirmation ? "bg-amber-50 border-amber-200 dark:bg-amber-950/30 dark:border-amber-900/40" : "bg-slate-50 border-slate-200 dark:bg-zinc-950/40 dark:border-zinc-800"}`}
      >
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-gray-500 mb-2">
              {t("wizard.goalMri.title")}
            </p>
            <h4 className="text-base font-bold text-gray-900 dark:text-white">
              {t("wizard.goalMri.heading")}
            </h4>
          ...
          <span
            className={`inline-flex max-w-full items-center justify-center self-start rounded-full px-3 py-1 text-center text-xs font-bold md:self-auto ${mri.readiness === "ready" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300" : "bg-white text-amber-700 border border-amber-200 dark:bg-zinc-900 dark:text-amber-300 dark:border-amber-900/40"}`}
          >
            {statusLabel}
          </span>
        </div>
  ...
  const renderDraftContent = (draft: any) => {
    ...
    const renderBody = () => {
        ...
        return (
          <div className="space-y-4">
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-blue-100 dark:border-blue-900/30">
              <h5 className="font-bold text-blue-600 text-sm mb-2">
                {t("pareto.vital")}
              </h5>
              {vital.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300">
                  {vital.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
                <p className="text-sm text-gray-400 dark:text-gray-500 italic">
                  {t("wizard.planStepsPlaceholder")}
                </p>
              )}
            </div>
            <div className="p-4 bg-gray-100 dark:bg-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700">
              <h5 className="font-bold text-gray-500 text-sm mb-2">
                {t("pareto.trivial")}
              </h5>
              {trivial.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-400">
                  {trivial.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
        ...
        return (
          <div className="space-y-4">
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200/80 dark:border-zinc-700/80">
              <h5 className="font-bold text-slate-600 dark:text-slate-400 text-sm mb-2">
                {t("fp.truths")}
              </h5>
              {truths.length > 0 ? (
            ...
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-blue-100 dark:border-blue-900/40">
              <h5 className="font-bold text-blue-600 dark:text-blue-400 text-sm mb-2">
                {t("fp.newApproach")}
              </h5>
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
                {newApproach || t("wizard.planStepsPlaceholder")}
              </p>
            </div>
          </div>
        );
      ...
      if (draft.type === "okr") {
        const hasKrs =
          Array.isArray(draft.keyResults) && draft.keyResults.length > 0;
        ...
        return (
          <div className="space-y-4">
            <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-100 dark:border-purple-900/40">
              <h5 className="font-bold text-purple-600 text-sm mb-2">
                {t("okr.northStar")}
              </h5>
              <p className="text-lg font-bold text-gray-900 dark:text-white leading-tight">
                {draft.objective || t("wizard.planStepsPlaceholder")}
              </p>
            </div>
            ...
            {initiative && (
              <div className="p-4 bg-purple-100/50 dark:bg-purple-900/30 rounded-xl border border-purple-200/60 dark:border-purple-800/40">
                <h5 className="font-bold text-purple-600 dark:text-purple-400 text-sm mb-1.5">
                  {t("okr.initiative")}
                </h5>
        ...
        return (
          <div className="space-y-4">
            <div className="p-4 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200/60 dark:border-amber-800/40">
              <h5 className="font-bold text-amber-700 dark:text-amber-400 text-sm mb-1.5">
                {t("wizard.mandalaCentralGoal")}
              </h5>
              <p className="text-sm font-medium text-gray-900 dark:text-white leading-tight">
                {goal || t("wizard.planStepsPlaceholder")}
              </p>
            </div>
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
              <h5 className="font-bold text-gray-500 dark:text-gray-400 text-sm mb-2">
                {t("wizard.mandalaCategories")}
              </h5>
              {categories.length > 0 ? (
                <ul className="space-y-1.5">
                  {categories.map(
                    (cat: { name?: string; steps?: string[] }, i: number) => (
                      <li
                        key={i}
                        className="flex items-center justify-between gap-2 text-sm text-gray-700 dark:text-gray-300"
                      >
                        <span className="truncate">
                          {cat.name || t("wizard.planStepsPlaceholder")}
                        </span>
      ...
      if (draft.type === "eisenhower") {
        ...
        const quads = [
          {
            key: "eisenhower.do",
            label: t("eisenhower.do"),
            items: q1,
            bg: "bg-red-50 dark:bg-red-950/20 border-red-200/60 dark:border-red-900/30",
          },
          {
            key: "eisenhower.schedule",
            label: t("eisenhower.schedule"),
            items: q2,
            bg: "bg-blue-50 dark:bg-blue-950/20 border-blue-200/60 dark:border-blue-900/30",
          },
          {
            key: "eisenhower.delegate",
            label: t("eisenhower.delegate"),
            items: q3,
            bg: "bg-amber-50 dark:bg-amber-950/20 border-amber-200/60 dark:border-amber-900/30",
          },
          {
            key: "eisenhower.eliminate",
            label: t("eisenhower.eliminate"),
            items: q4,
            bg: "bg-gray-100 dark:bg-zinc-800/80 border-gray-200 dark:border-zinc-700",
          },
        ];
        return (
          <div className="grid grid-cols-2 gap-3">
            {quads.map((q) => (
              <div key={q.key} className={`p-3 rounded-xl border ${q.bg}`}>
                <h5 className="font-bold text-gray-700 dark:text-gray-300 text-xs mb-1.5 truncate">
                  {q.label}
                </h5>
        ...
        return (
          <div className="space-y-4">
            <div className="p-4 bg-red-50 dark:bg-red-950/30 rounded-xl border border-red-200/60 dark:border-red-900/40">
              <h5 className="font-bold text-red-700 dark:text-red-400 text-sm mb-1.5">
                The Challenge
              </h5>
              <p className="text-sm font-medium text-gray-900 dark:text-white leading-tight line-clamp-2">
                {challenge || t("wizard.planStepsPlaceholder")}
              </p>
            </div>
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
              <h5 className="font-bold text-gray-500 dark:text-gray-400 text-sm mb-1.5">
                The Failure Gap
              </h5>
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-snug line-clamp-2">
                {gap || t("wizard.planStepsPlaceholder")}
              </p>
            </div>
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
              <h5 className="font-bold text-gray-500 dark:text-gray-400 text-sm mb-1.5">
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-snug line-clamp-2">
                {purification || t("wizard.planStepsPlaceholder")}
              </p>
            </div>
          </div>
        );
          ...
          <div className="space-y-4">
            {purpose && (
              <div className="p-4 bg-rose-50 dark:bg-rose-950/30 rounded-xl border border-rose-200/60 dark:border-rose-900/40">
                <h5 className="font-bold text-rose-700 dark:text-rose-400 text-sm mb-1.5">
                  {t("ikigai.purpose")}
                </h5>
                <p className="text-sm font-medium text-gray-900 dark:text-white leading-tight">
                  {purpose}
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                {
                  key: "ikigai.goodAt",
                  label: t("ikigai.goodAt"),
                  val: goodAt,
                },
                {
                  key: "ikigai.worldNeeds",
                  label: t("ikigai.worldNeeds"),
                  val: worldNeeds,
                },
                {
                  key: "ikigai.paidFor",
                  label: t("ikigai.paidFor"),
                  val: paidFor,
                },
              ].map(({ label, val }) => (
                <div
                  key={label}
                  className="p-3 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800"
                >
                  <h5 className="font-bold text-gray-500 dark:text-gray-400 text-xs mb-1 truncate">
                    {label}
                  </h5>
                  <p className="text-sm text-gray-700 dark:text-gray-300 leading-snug line-clamp-2">
                    {val || t("wizard.planStepsPlaceholder")}
                  </p>
                </div>
              ))}
            </div>
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="p-4 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200/60 dark:border-amber-900/40">
              <h5 className="font-bold text-amber-700 dark:text-amber-400 text-sm mb-2">
                {t("dsss.selection")}
              </h5>
              {selection.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                  {selection.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
            ...
            <div className="p-4 bg-yellow-50/80 dark:bg-yellow-950/20 rounded-xl border border-yellow-200/60 dark:border-yellow-900/30">
              <h5 className="font-bold text-yellow-700 dark:text-yellow-400 text-sm mb-2">
                {t("dsss.sequence")}
              </h5>
              {sequence.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                  {sequence.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
            ...
            {stakes && (
              <div className="p-4 bg-red-50/80 dark:bg-red-950/20 rounded-xl border border-red-200/60 dark:border-red-900/30">
                <h5 className="font-bold text-red-700 dark:text-red-400 text-sm mb-2">
                  {t("dsss.stakes")}
                </h5>
        ...
        return (
          <div className="space-y-4">
            <div className="p-4 bg-sky-50 dark:bg-sky-950/30 rounded-xl border border-sky-200/60 dark:border-sky-900/40">
              <h5 className="font-bold text-sky-700 dark:text-sky-400 text-sm mb-2">
                {t("gps.goal")}
              </h5>
              <p className="text-sm font-medium text-gray-900 dark:text-white leading-tight">
                {goal || t("wizard.planStepsPlaceholder")}
            ...
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
              <h5 className="font-bold text-sky-600 dark:text-sky-400 text-sm mb-2">
                {t("gps.plan")}
              </h5>
              {plan.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                  {plan.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
            ...
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
              <h5 className="font-bold text-cyan-600 dark:text-cyan-400 text-sm mb-2">
                {t("gps.system")}
              </h5>
              {system.length > 0 ? (
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                  {system.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
            ...
            {antiGoals.length > 0 && (
              <div className="p-4 bg-amber-50/80 dark:bg-amber-950/20 rounded-xl border border-amber-200/60 dark:border-amber-900/30">
                ...
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                  {antiGoals.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              </div>
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800">
              ...
              {hasPlan ? (
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300">
                  {draft.plan.map((v: string, i: number) => (
                    <li key={i}>{v}</li>
                  ))}
                </ul>
              ) : (
      ...
      const eisenhowerLabels: Record<string, string> = {
        q1: "eisenhower.do",
        q2: "eisenhower.schedule",
        q3: "eisenhower.delegate",
        q4: "eisenhower.eliminate",
      };
      return (
        <div className="space-y-4">
          {Object.entries(draft).map(([key, val]) => {
            ...
            const labelKey =
              draft.type === "rpm" &&
              (key === "result" || key === "purpose" || key === "plan")
                ? key === "result"
                  ? "rpm.outcome"
                  : key === "purpose"
                    ? "rpm.purpose"
                    : "rpm.map"
                : draft.type === "eisenhower"
                  ? eisenhowerLabels[key]
                  : null;
                ...
                {Array.isArray(val) ? (
                  <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300 space-y-0.5">
                    {(val as string[]).map((v, i) =>
                      typeof v === "string" ? <li key={i}>{v}</li> : null,
                    )}
                  </ul>
                ) : (
                  <p className="text-sm text-black dark:text-white">
    ...
    return (
      <div
        className={`space-y-6 transition-all duration-500 ${draftPulse ? "scale-[1.02] ring-2 ring-blue-400/50 rounded-2xl shadow-[0_0_30px_rgba(59,130,246,0.2)] bg-blue-50/30" : ""}`}
      >
        <div className="flex items-start justify-between border-b border-gray-100 dark:border-zinc-800 pb-4">
          <div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <span className="capitalize">
                {draft.type?.replace("-", " ") || "Plan"}
              </span>
            </h3>
            <p className="text-xs text-gray-400 mt-1">
              {t("wizard.buildingPlanUpdates")}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-500 mt-2 leading-snug">
              {t("wizard.draftPanelPurpose")}
            </p>
          </div>
  ...
  return (
    <>
      ...
      >
        <ScrollArea
          className={`flex-1 min-h-0 w-full rounded-l-2xl overflow-hidden ${!draftResult?.isTeaser && draftResult?.type ? "" : "pointer-events-none opacity-80"}`}
        >
            ...
            <div className="pt-4 pb-2">
              <div className="flex items-center gap-2 text-gray-400 uppercase tracking-widest text-xs font-bold">
                ...
                {isTyping || isAgentRunning
                  ? t("wizard.liveDraft")
                  : t("wizard.yourBlueprint")}
              </div>
              {!draftResult?.isTeaser &&
                draftResult?.type &&
                onFinalize &&
                !isTyping &&
                !isAgentRunning && (
          ...
          >
            <div className="flex-none p-4 border-b border-gray-100 dark:border-zinc-800 flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 bg-gray-100 dark:bg-zinc-800 rounded-full cursor-pointer hover:bg-gray-200 dark:hover:bg-zinc-700 transition-colors"
