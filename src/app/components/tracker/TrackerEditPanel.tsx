...
interface TrackerEditPanelProps {
  ...
  onSave: (
    ...
    updatedTracker: Partial<BlueprintTracker>,
  ) => Promise<void>;
}

export function TrackerEditPanel({
  isOpen,
  onClose,
  blueprint,
  tracker,
  onSave,
}: TrackerEditPanelProps) {
  const { t } = useLanguage();
  ...
  const [reminders, setReminders] = useState<
    { id?: string; time: string; days: string[] }[]
  >([]);
  const [loadingReminders, setLoadingReminders] = useState(false);
  const [savingsEnabled, setSavingsEnabled] = useState(
    tracker.savings_enabled || false,
  );
  const [savingsBaseline, setSavingsBaseline] = useState(
    tracker.savings_baseline || 0,
  );
  const [savingsUnit, setSavingsUnit] = useState(tracker.savings_unit || "$");
  const [tagsText, setTagsText] = useState(
    tracker.tags ? tracker.tags.join(", ") : "",
  );
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  title={t("tracker.trackingQuestion")}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white transition-all"
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  <Tag size={16} />{" "}
                  {t("tracker.tags") || "Tags (comma separated)"}
                </label>
                <input
                  type="text"
                  placeholder={
                    t("tracker.tagsPlaceholder") ||
                    "e.g. health, project, daily"
                  }
                  value={tagsText}
                  onChange={(e) => setTagsText(e.target.value)}
                  title={t("tracker.tags") || "Tags"}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white transition-all"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  title={t("tracker.frequency")}
                  aria-label={t("tracker.frequency")}
                ...
                >
                  ...
                  <option value="weekly">
                    {t("tracker.frequency.weekly")}
                  </option>
                  <option value="custom">
                    {t("tracker.frequency.custom")}
                  </option>
                </select>
              </div>
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-gray-200 dark:border-zinc-800">
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <PiggyBank size={16} />{" "}
                    {t("tracker.savings") || "Track Savings"}
                  </span>
                </label>
                  ...
                  >
                    <div className="flex gap-4">
                      <div className="flex-1 space-y-2">
                        <label className="text-xs font-bold text-gray-600 dark:text-gray-400 uppercase text-left block">
                          Amount per entry
                        </label>
                        <input
                          type="number"
                          ...
                          onChange={(e) =>
                            setSavingsBaseline(parseFloat(e.target.value) || 0)
                          }
                          title="Amount per entry"
              ...
              <div className="space-y-4 pt-6 mt-6 border-t border-gray-200 dark:border-zinc-800">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <Clock size={16} />{" "}
                    {t("tracker.reminders.title") || "Reminders"}
                  </span>
                  {!loadingReminders && reminders.length < 3 && (
                    <button
                      ...
                      title={t("tracker.reminders.add")}
                      aria-label={t("tracker.reminders.add")}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <input
                              ...
                              title={
                                t("tracker.reminders.time") || "Reminder time"
                              }
                                ...
                                <button
                                  ...
                                  className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex justify-center items-center text-sm font-bold transition-colors cursor-pointer ${
                                    isSelected
                                      ...
                                      : "bg-white dark:bg-zinc-900 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-zinc-700"
                                  }`}
                                >
                                  {day.label}
                                </button>
