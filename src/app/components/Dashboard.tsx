...
const FRAMEWORK_THEMES: Record<string, { bg: string, border: string, iconBg: string, iconColor: string, icon: any }> = {
  ...
  'dsss': { bg: 'from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20', border: 'border-orange-200 dark:border-orange-800', iconBg: 'bg-orange-100 dark:bg-orange-900/40', iconColor: 'text-orange-600 dark:text-orange-400', icon: Layers },
  'first-principles': { bg: 'from-zinc-50 to-slate-50 dark:from-zinc-900/20 dark:to-slate-900/20', border: 'border-zinc-200 dark:border-zinc-800', iconBg: 'bg-zinc-200 dark:bg-zinc-800', iconColor: 'text-zinc-700 dark:text-zinc-300', icon: Target },
  'gps': { bg: 'from-sky-50 to-cyan-50 dark:from-sky-900/20 dark:to-cyan-900/20', border: 'border-sky-200 dark:border-sky-800', iconBg: 'bg-sky-100 dark:bg-sky-900/40', iconColor: 'text-sky-600 dark:text-sky-400', icon: Layout },
  'mandalas': { bg: 'from-yellow-50 to-amber-50 dark:from-yellow-900/20 dark:to-amber-900/20', border: 'border-yellow-200 dark:border-yellow-800', iconBg: 'bg-yellow-100 dark:bg-yellow-900/40', iconColor: 'text-yellow-600 dark:text-yellow-400', icon: LayoutGrid },
  'general': { bg: 'from-gray-50 to-zinc-50 dark:from-zinc-900/20 dark:to-zinc-800/20', border: 'border-gray-200 dark:border-zinc-800', iconBg: 'bg-gray-100 dark:bg-zinc-800', iconColor: 'text-gray-600 dark:text-gray-400', icon: Layout },
...
export function Dashboard({
  ...
  useEffect(() => {
    ...
    if (savedView === 'grid' || savedView === 'list' || savedView === 'reports') {
  ...
  const togglePin = (e: React.MouseEvent, id: string) => {
      ...
      try {
          ...
          if (err.name === 'QuotaExceededError' || err.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
              toast.error(t('errors.storageFull') || "Storage full: Cannot pin more items. Please unpin some first.", { duration: 15000 });
              // Rollback state
  ...
  React.useEffect(() => {
    if (supabase) {
        supabase.auth.getUser().then(({ data: { user } }) => {
            if (user) {
                supabase.from('profiles').select('branding_logo_url, branding_color, tier, metadata').eq('user_id', user.id).single()
                .then(({ data }) => {
                    if (data) {
                        setBranding({
    ...
    <section className="px-6 pb-24 pt-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="mb-10 space-y-4">
          ...
          <div className="flex justify-between items-center mt-4 border-t border-gray-100 dark:border-zinc-800 pt-4">
              <div className="text-sm font-bold text-gray-500">{filteredBlueprints.length} {filteredBlueprints.length === 1 ? 'Plan' : 'Plans'}</div>
              <div className="flex bg-gray-100 dark:bg-zinc-800/80 p-1 rounded-xl">
                  <button onClick={() => handleViewModeChange('grid')} className={`p-1.5 rounded-lg flex items-center justify-center transition-colors ${viewMode === 'grid' ? 'bg-white dark:bg-zinc-700 shadow-sm text-black dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`} title="Grid View"><LayoutGrid size={16} /></button>
                  <button onClick={() => handleViewModeChange('list')} className={`p-1.5 rounded-lg flex items-center justify-center transition-colors ${viewMode === 'list' ? 'bg-white dark:bg-zinc-700 shadow-sm text-black dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`} title="List View"><List size={16} /></button>
                  <button onClick={() => handleViewModeChange('reports')} className={`p-1.5 rounded-lg flex items-center justify-center transition-colors ${viewMode === 'reports' ? 'bg-white dark:bg-zinc-700 shadow-sm text-black dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`} title="Reports View"><BarChart size={16} /></button>
              </div>
          </div>
      </div>
                                ...
                                if (tracker.plan_kind === 'finite') {
                                    ...
                                    return trackerPrefs?.show_score !== false ? (
                                        <div className="flex items-center gap-1">
                                            ...
                                            <span className="text-[10px] font-bold text-gray-500">{completed}/{totalSteps}</span>
                                        </div>
                                    ) : null;
                                }
              ...
              >
                ...
                <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto overflow-hidden">
                    <div className="flex-1 md:flex-none flex justify-start md:justify-center overflow-x-auto scrollbar-hide">
                        {trackerPrefs?.show_heatmap !== false && (
                            <TrackerHeatmap logs={logsMap[bp.id] || []} daysBack={10} color={trackers[bp.id]?.color} className="shrink-0" />
                        )}
                    </div>
                  ...
                  <div className="flex-1 flex flex-col justify-end">
                      {trackerPrefs?.show_heatmap !== false && (
                          <div className="bg-gray-50 dark:bg-zinc-800/50 p-4 rounded-2xl">
                              ...
                              <TrackerHeatmap logs={logsMap[bp.id] || []} daysBack={28} color={trackers[bp.id]?.color} className="w-full" />
                          </div>
                      )}
                  </div>
              </motion.div>
