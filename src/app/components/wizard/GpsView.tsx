...
function normalizeGpsResult(result: GpsResult): {
  goal: string;
  plan: string[];
  system: string[];
  anti_goals: string[];
  ...
  const goal =
    typeof result.goal === "string" && result.goal.trim()
      ? result.goal.trim()
      : "";
  const plan = Array.isArray(result.plan)
    ? result.plan.filter((s): s is string => typeof s === "string")
    : [];
  const system = Array.isArray(result.system)
    ? result.system.filter((s): s is string => typeof s === "string")
    : [];
  const anti_goals = Array.isArray(result.anti_goals)
    ? result.anti_goals.filter((s): s is string => typeof s === "string")
    : [];
...
export const GpsView: React.FC<GpsViewProps> = ({ result, updateResult }) => {
  ...
  const { goal, plan, system, anti_goals, shortTitle } =
    normalizeGpsResult(result);
  ...
  return (
    <div className="mt-6 w-full max-w-4xl mx-auto px-4 pb-16 sm:pb-20">
          ...
          {STEP_CONFIG.map((step, i) => {
            ...
            return (
              <React.Fragment key={step.key}>
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r ${step.gradient} text-white text-sm font-bold shadow-lg`}
                >
                ...
                {i < STEP_CONFIG.length - 1 && (
                  <div
                    className="hidden sm:block w-6 h-0.5 bg-gray-300 dark:bg-zinc-600 rounded"
                    aria-hidden
                  />
                )}
              </React.Fragment>
      ...
      >
        <div
          className={`absolute top-0 left-0 right-0 h-1 ${STEP_CONFIG[0].accent}`}
        />
        ...
        <div className="relative z-10 p-8 md:p-10">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              <h2 className="text-xl font-bold text-gray-900 dark:text-white mt-1 sr-only">
                {t("gps.goal")}
              </h2>
            </div>
          </div>
        ...
        <div className="relative z-10 flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t("gps.planDesc") || "The route to get there"}
            </p>
          </div>
        </div>
        {displayPlan.length > 0 ? (
          ...
          <div className="text-gray-500 dark:text-gray-400 italic py-4">
            <EditableList
              items={[""]}
              onChange={(val) =>
                updateResult(["plan"], val.filter(Boolean).length ? val : [""])
              }
              itemClassName="text-base text-gray-700 dark:text-gray-200 border-l-4 border-cyan-400 dark:border-cyan-600 pl-4 py-3 bg-cyan-50/50 dark:bg-cyan-950/20 rounded-r-xl"
      ...
      >
        ...
        <div className="relative z-10 flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {t("gps.systemDesc") ||
                "Habits & environment that keep you on track"}
            </p>
          </div>
        </div>
